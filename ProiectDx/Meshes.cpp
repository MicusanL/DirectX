...
void InitiateCamera()
{
	...
	D3DXVECTOR3 vEyePt(0.0f, 3.0f, -200.0f);
...
HRESULT InitGeometry()
{
	...
	if (FAILED(D3DXLoadMeshFromX("naruto.x", D3DXMESH_SYSTEMMEM,
		d3dDevice, NULL,
		&pD3DXMtrlBuffer, NULL, &NumMaterials,
		&Mesh)))
	{
		// If model is not in current folder, try parent folder
		if (FAILED(D3DXLoadMeshFromX("..\\naruto.x", D3DXMESH_SYSTEMMEM,
			d3dDevice, NULL,
			&pD3DXMtrlBuffer, NULL, &NumMaterials,
			&Mesh)))
		{
			MessageBox(NULL, "Could not find naruto.x", "Meshes.exe", MB_OK);
			return E_FAIL;
		}
	}
